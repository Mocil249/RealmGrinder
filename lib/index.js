    ...
    this.get_tiers = function(save) {
	  ...
	  for (var spell of spells) {
        ...
        else
        {
            ...
            {
                spell.text = "当技能可视时，下一层数将解锁";
            }
            else
            {          
                spell.text = "下一层数解锁于 " + util.render.time(spell.time2tier);
            }
        }
	  ...
	  while (currentTiers < maxTiers)
	  {
		  ...
		  timeLineText.push(util.render.time(timeLineTimeSpent) + ": " + spell2tier.name + " 将到层 " + (spell2tier.unlocked + 2));
